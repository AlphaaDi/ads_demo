...
def crop_logo(ret):
#   ret_gray = cv2.cvtColor(ret, cv2.COLOR_BGRA2GRAY)
    bin_mask = (ret[:,:,3]).astype(np.uint8) 
...
def select_best_plane(masks, planes, reference_vector=(0,0,1), trashhold=0.6):
#   masks, planes = map(deepcopy, [masks, planes])
    idx_max = None
    max_rad = 0
    np_ref_vector = np.array(reference_vector)
    for i in range(len(masks)):
        mask = masks[i]
        plane_norm = planes[i]
        
        plane_norm[0], plane_norm[1], plane_norm[2] = plane_norm[0], plane_norm[2], plane_norm[1]
        v_norm = normalize(plane_norm[None])[0]
        if v_norm @ np_ref_vector < trashhold:
            continue

        _, r, _ = get_center_rad_inscribed(mask)
        if r > max_rad:
            idx_max = i
            max_rad = r
    return idx_max
...
#   img, masks, planes, logo = map(deepcopy, [img, masks, planes, logo])
    best_idx = select_best_plane(masks, planes)
    all_canvas = []
    for mask, plane_norm in zip(masks, planes):
        mask = mask.astype(np.uint8)
        ...
        canva = np.zeros((img.shape[0], img.shape[1], 4)).astype(np.uint8)
        x1,y1 = center[0] - w//2, center[1] - w//2

        w_crop, h_crop, _ = crop_r.shape
        canva[y1 : y1 + w_crop, x1 : x1 + h_crop] = crop_r
        canva_l = canva[:,:,3]
        canva_3l = cv2.cvtColor(canva_l, cv2.COLOR_GRAY2BGR)
        canva[:,:,:3] =  (canva_3l / 255 * canva[:,:,:3] ).astype(np.uint8)
        ...
        all_canvas.append(canva)

#       cv2.imshow('canva', canva[:,:,3])
        canva_l = canva[:,:,3]

        canva_3l = cv2.cvtColor(canva_l, cv2.COLOR_GRAY2BGR)
#       img[canva_l > 0] = canva[:,:,:3][(canva_l > 0)]
        img = canva_3l / 255 * canva[:,:,:3] + (1 - canva_3l / 255) * img
        img = img.astype(np.uint8)
        x_norm, y_norm = v_norm[:2]
        x_norm, y_norm = -x_norm, -y_norm
        logo_mask = (canva_l > 0).astype(np.uint8)
        arrow_begin = get_mask_center(logo_mask)
        arrow_end = arrow_begin + np.array([x_norm, y_norm]) * 100
        arrow_end = arrow_end.astype(int)
        cv2.arrowedLine(img, arrow_begin, arrow_end, (0, 255, 0), 2)
    
    return all_canvas, img

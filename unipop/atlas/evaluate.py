...
from unipop.atlas.loss_utils import get_rigidity_loss, get_optical_flow_loss_all, get_optical_flow_alpha_loss_all
...
def get_mapping_area(model_F_mapping, F_alpha, mask_frames, resx, number_of_frames, uv_shift, device,
    ...
    batch_size = 400000
    num_batches = int(np.ceil(relis_i.shape[0] / batch_size))

    relisa = np.array_split(relis_i.numpy(), num_batches)
    reljsa = np.array_split(relis_j.numpy(), num_batches)
    relfsa = np.array_split(relis_f.numpy(), num_batches)
    ...
    with torch.no_grad():
        for i in range(num_batches):
            ...
            end_idx = min((i + 1) * batch_size, relis_i.shape[0])
            ...
            uv = model_F_mapping(torch.cat((reljs, relis, relfs), dim=1).to(device)).cpu()
            alpha = F_alpha(torch.cat((reljs, relis, relfs), dim=1).to(device)).cpu().squeeze()

            if invert_alpha:
                alpha = -alpha

            alpha_thresh_mask = alpha > alpha_thresh
            if torch.any(alpha_thresh_mask):
                uv = uv * 0.5 + uv_shift

                curminx, _ = torch.min(uv[alpha_thresh_mask, 0], dim=0)
                curminy, _ = torch.min(uv[alpha_thresh_mask, 1], dim=0)
                curmaxx, _ = torch.max(uv[alpha_thresh_mask, 0], dim=0)
                curmaxy, _ = torch.max(uv[alpha_thresh_mask, 1], dim=0)
...
def evaluate_model(model_F_atlas, resx, resy, number_of_frames, model_F_mapping1, model_F_mapping2, model_alpha,
    ...
    fpath = os.path.join(evaluation_folder, 'texture_edit1.png')
    mpimg.imsave(fpath,
                 (masks1[:, :, np.newaxis] * edited_tex1.numpy() * (255)).astype(np.uint8))
    fpath = os.path.join(evaluation_folder, 'texture_orig1.png')
    mpimg.imsave(fpath,
                 (masks1[:, :, np.newaxis] * texture_orig1.numpy() * (255)).astype(np.uint8))
    fpath = os.path.join(evaluation_folder,'texture_edit2.png')
    mpimg.imsave(fpath,
                 (masks2[:, :, np.newaxis] * edited_tex2.numpy() * (255)).astype(np.uint8))
    if show_atlas_alpha:
        fpath = os.path.join(evaluation_folder,'texture_orig1_alpha.png')
        mpimg.imsave(fpath,
                     (np.concatenate((texture_orig1.numpy(), masks1_alpha[:, :, np.newaxis]), axis=2) * (255)).astype(
                         np.uint8))
        fpath = os.path.join(evaluation_folder, 'texture_orig2_alpha.png')
        mpimg.imsave(fpath,
                     (np.concatenate((texture_orig2.numpy(), masks2_alpha[:, :, np.newaxis]), axis=2) * (255)).astype(
                         np.uint8))

    fpath = os.path.join(evaluation_folder, 'texture_orig2.png')
    mpimg.imsave(fpath,
                 (masks2[:, :, np.newaxis] * texture_orig2.numpy() * (255)).astype(np.uint8))

    fpath = os.path.join(evaluation_folder, f'edited1_{vid_name}.mp4')
    writer_t_edited1 = imageio.get_writer(
        fpath, fps=10)
    fpath = os.path.join(evaluation_folder, f'edited1_tex_{vid_name}.mp4')
    writer_t_edited1_tex = imageio.get_writer(
        fpath, fps=10)
    fpath = os.path.join(evaluation_folder, f'edited2_tex_{vid_name}.mp4')
    writer_t_edited2_tex = imageio.get_writer(
        fpath, fps=10)

    fpath = os.path.join(evaluation_folder, f'edited2_{vid_name}.mp4')
    writer_t_edited2 = imageio.get_writer(
        fpath, fps=10)
    fpath = os.path.join(evaluation_folder, f'alpha_{vid_name}.mp4')
    writer_alpha = imageio.get_writer(
        fpath, fps=10)

    fpath = os.path.join(evaluation_folder, f'reconstruction_{vid_name}.mp4')
    writer_im_rec = imageio.get_writer(
        fpath, fps=10)

    fpath = os.path.join(evaluation_folder, f'residuals_{vid_name}.mp4')
    writer_residuals = imageio.get_writer(
        fpath, fps=10)

    fpath = os.path.join(evaluation_folder, f'alpha_vs_mask_{vid_name}.mp4')
    writer_alpha_vs_mask_rcnn = imageio.get_writer(
        fpath, fps=10)

    fpath = os.path.join(evaluation_folder, f'edit_{vid_name}.mp4')
    writer_edit = imageio.get_writer(
        fpath, fps=10)

    fpath = os.path.join(evaluation_folder, f'uv_1_{vid_name}.mp4')
    writer_uv_1 = imageio.get_writer(
        fpath,
        fps=10)

    fpath = os.path.join(evaluation_folder, 'uv_1_masked_{vid_name}.mp4')
    writer_uv_1_masked = imageio.get_writer(
        fpath,
        fps=10)

    fpath = os.path.join(evaluation_folder, f'uv_2_{vid_name}.mp4')
    writer_uv_2 = imageio.get_writer(
        fpath,
        fps=10)
    fpath = os.path.join(evaluation_folder, f'checkerboard_1_{vid_name}.mp4')
    writer_checkerboard_1 = imageio.get_writer(
        fpath,
        fps=10)
    fpath = os.path.join(evaluation_folder, f'checkerboard_2_{vid_name}.mp4')
    writer_checkerboard_2 = imageio.get_writer(
        fpath,
        fps=10)

    fpath = os.path.join(evaluation_folder, f'global_info_{vid_name}.mp4')
    ...
    fpath = os.path.join(evaluation_folder, f'PSNR_{pnsrs.mean()}')
    file1 = open(fpath, "a")
    file1.close()
    if save_checkpoint:

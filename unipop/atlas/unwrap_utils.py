...
def load_input_data(resy, resx, maximum_number_of_frames, data_folder, use_mask_rcnn_bootstrapping,  filter_optical_flow,
                    vid_root, vid_name):
    out_flow_dir = vid_root / vid_name /'flowdir'
    maskrcnn_dir = vid_root / vid_name /'maskdir' 

    imgs_dir = vid_root / vid_name / 'images'

    input_files = sorted(list(imgs_dir.glob('*.jpg')) + list(imgs_dir.glob('*.png')))

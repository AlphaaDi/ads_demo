...
from unipop.atlas.evaluate import get_high_res_texture, get_colors,get_mapping_area
from unipop.atlas.implicit_neural_networks import IMLP


device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")

def apply_edit_for_layer(model_F_atlas, resx, resy, number_of_frames,
                         video_frames, video_original,
                         model_F_mapping2, model_F_mapping, model_alpha, 
                         output_folder_final, mask_frames, vid_name,
                         texture_edit_im, alpha_im, fgrnd):
    ...
    if fgrnd:
        edited_tex1_only_edit = torch.from_numpy(texture_edit_im)
        # save the given edits:
        imageio.imwrite(
            "%s/texture_edit_user1.png" % output_folder_final,(
            np.concatenate((texture_edit_im,alpha_im[:,:,np.newaxis]),axis=2)*255).astype(np.uint8))
        edited_tex1 = torch.from_numpy(1 - alpha_im).unsqueeze(-1) * texture_orig1 + torch.from_numpy(
            alpha_im).unsqueeze(-1) * texture_edit_im
        edited_tex1_only_edit = torch.cat((edited_tex1_only_edit, torch.from_numpy(alpha_im).unsqueeze(-1)), dim=-1)
    else :
        edited_tex1_only_edit = torch.from_numpy(texture_edit_im)
        imageio.imwrite(
            "%s/texture_edit_user2.png" % output_folder_final,(
            np.concatenate((texture_edit_im,alpha_im[:,:,np.newaxis]),axis=2)*255).astype(np.uint8))
        edited_tex1 = torch.from_numpy(1 - alpha_im).unsqueeze(-1) * texture_orig2 + torch.from_numpy(
            alpha_im).unsqueeze(-1) * texture_edit_im
    ...
    masks = np.zeros((edited_tex1.shape[0], edited_tex1.shape[1]))
    ...
    with torch.no_grad():
        ...
        relis = relis_i / (larger_dim / 2) - 1
        reljs = reljs_i / (larger_dim / 2) - 1
        relis = relis.unsqueeze(1)
        reljs = reljs.unsqueeze(1)
        
        f_values = torch.arange(0, number_of_frames).unsqueeze(0) / (number_of_frames / 2.0) - 1
        f_grid = f_values.repeat(len(relis), 1)
        
        input_tensor = torch.stack((reljs, relis), dim=-1).repeat(1, number_of_frames)
        input_tensor = torch.cat((input_tensor, f_grid.unsqueeze(-1)), dim=-1).to(device)
    
        uv_temp1 = model_F_mapping(input_tensor)
        alpha = 0.5 * (model_alpha(input_tensor) + 1.0)
        alpha = alpha * 0.99
        alpha = alpha + 0.001
    
        uv_temp1 = uv_temp1.detach().cpu()
        uv_temp1_x, uv_temp1_y = uv_temp1[:, 0] * 0.5 + 0.5, uv_temp1[:, 1] * 0.5 + 0.5
    
        if fgrnd:
            rgb_only_edit, pointsx1, pointsy1, relevant1_only_edit = get_colors(
                1000,
                minx, minx + edge_size, miny, miny + edge_size,
                uv_temp1_x, uv_temp1_y,
                edited_tex1_only_edit)
        else:
            rgb_only_edit, pointsx1, pointsy1, relevant1_only_edit = get_colors(
                1000,
                minx2, minx2 + edge_size2, miny2, miny2 + edge_size2,
                uv_temp1_x - 0.5, uv_temp1_y - 0.5,
                edited_tex1_only_edit)
    
        pointsx1_ceil, pointsy1_ceil = np.ceil(pointsx1).astype(np.int64), np.ceil(pointsy1).astype(np.int64)
        pointsx1_floor, pointsy1_floor = np.floor(pointsx1).astype(np.int64), np.floor(pointsy1).astype(np.int64)
        mask_indices = np.stack([(pointsy1_ceil, pointsx1_ceil),
                                 (pointsy1_floor, pointsx1_floor),
                                 (pointsy1_floor, pointsx1_ceil),
                                 (pointsy1_ceil, pointsx1_floor)], axis=1)
        try:
        ...
        alpha_reconstruction[relis_i, reljs_i, :] = alpha[:, 0].detach().cpu().numpy()
    
        only_mapped_texture[relis_i[relevant1_only_edit], reljs_i[relevant1_only_edit], :, :] = rgb_only_edit
    
        _edit_cur = only_mapped_texture[:, :, :3, :]  # denoted in the paper by c_{ef}
        _edit_cur_alpha = only_mapped_texture[:, :, 3, :][..., np.newaxis]  # denoted by \alpha_{ef}
    
        video_frame_cur = video_frames.cpu().clone().numpy()  
    
    
        # Equation (15):
        video_frame_cur_edited1 = _edit_cur * (_edit_cur_alpha) + video_frame_cur * (
                    1 - _edit_cur_alpha) #\bar{c}_b
    
        cur_alpha = alpha_reconstruction[:, :, :][:, :, np.newaxis]
    
        # Equation (16):
        if fgrnd:
            foreground_edit_output = video_frame_cur_edited1 * cur_alpha  
        else:
            foreground_edit_output = (1 - cur_alpha) * video_frame_cur_edited1
    
        rgb_edit_video[:, :, :, :] = foreground_edit_output
    


#    with torch.no_grad():
#        relis_i, reljs_i = torch.where(torch.ones(resy, resx) > 0)
#        relisa = np.array_split(relis_i.numpy(), np.ceil(relis_i.shape[0] / 600000))
#        reljsa = np.array_split(reljs_i.numpy(), np.ceil(relis_i.shape[0] / 600000))
#        relis = torch.from_numpy(relisa[0]).unsqueeze(1) / (larger_dim / 2) - 1
#        reljs = torch.from_numpy(reljsa[0]).unsqueeze(1) / (larger_dim / 2) - 1
#        for f in range(number_of_frames):
#            print(f)
#
##           for i in range(len(relisa)):
#
#            uv_temp1 = model_F_mapping(
...
#            if fgrnd:
#                rgb_only_edit, pointsx1, pointsy1, relevant1_only_edit = get_colors(1000,
#                                                                    minx, minx + edge_size, miny,
#                                                                    miny + edge_size,
#                                                                    uv_temp1[:, 0] * 0.5 + 0.5,
#                                                                    uv_temp1[:, 1] * 0.5 + 0.5,
#                                                                    edited_tex1_only_edit)
...
#
#            try:
#                masks[np.ceil(pointsy1).astype((np.int64)), np.ceil(pointsx1).astype((np.int64))] = 1
#                masks[np.floor(pointsy1).astype((np.int64)), np.floor(pointsx1).astype((np.int64))] = 1
#                masks[np.floor(pointsy1).astype((np.int64)), np.ceil(pointsx1).astype((np.int64))] = 1
#                masks[np.ceil(pointsy1).astype((np.int64)), np.floor(pointsx1).astype((np.int64))] = 1
#            except Exception:
#                pass
#
#
#            alpha_reconstruction[relisa[0], reljsa[0], f] = alpha[:, 0].detach().cpu().numpy()
#
#            # save the video pixels of the edits from the two atlases
#            only_mapped_texture[relisa[0][relevant1_only_edit], reljsa[0][relevant1_only_edit], :,f] = rgb_only_edit
#
#            # see details in Section 3.4 in the paper
#            _edit_cur = only_mapped_texture[:, :, :3, f] # denoted in the paper by c_{ef}
#            _edit_cur_alpha = only_mapped_texture[:, :, 3, f][:, :, np.newaxis] # denoted by \alpha_{ef}
#
#
...
#            rgb_edit_video[:, :, :, f] = foreground_edit_output
    ...
    for i in range(number_of_frames):
        print(i)
        cur_or_frame = video_original[i]
        cur_alpha = cv2.resize(only_mapped_texture[:, :, 3, i][:,:,np.newaxis], (cur_or_frame.shape[1], cur_or_frame.shape[0]))
        cur_edit_frame = (rgb_edit_video[:,:,:,i] * 255).astype(np.uint8)
        cur_edit_frame = cv2.resize(cur_edit_frame, (cur_or_frame.shape[1], cur_or_frame.shape[0]))

        cur_alpha = cv2.cvtColor(cur_alpha.astype(np.float32), cv2.COLOR_GRAY2BGR)
        res_frame = cur_edit_frame * cur_alpha + (1 - cur_alpha) * cur_or_frame
        res_frame = res_frame.astype(np.uint8) 
        
        writer_edit.append_data(res_frame)
...
def apply_edit(model_F_atlas, resx, resy, number_of_frames, model_F_mapping1, model_F_mapping2, model_alpha,
    ...
    for i in range(number_of_frames):
        print(i)
        cur_or_frame = video_original[i]
        cur_alpha = cv2.resize(only_mapped_texture2[:, :, 3, i][:,:,np.newaxis], (cur_or_frame.shape[1], cur_or_frame.shape[0]))
        cur_edit_frame = (rgb_edit_video[:,:,:,i] * 255).astype(np.uint8)
        cur_edit_frame = cv2.resize(cur_edit_frame, (cur_or_frame.shape[1], cur_or_frame.shape[0]))
...
def texture_edit_from_frame_edit(edit_frame, f, model_F_mapping1, model_F_mapping2, model_alpha, number_of_frames,
    ...
    maxx2, minx2, maxy2, miny2, edge_size2 = get_mapping_area(model_F_mapping2, model_alpha, mask_frames > -1, larger_dim,
                                                              number_of_frames,
                                                              torch.tensor([-0.5, -0.5]),device, invert_alpha=True)
#   print(maxx2, minx2, maxy2, miny2, edge_size2)
...
def main(training_folder, frame_edit, frames_folder, mask_folder, frame_edit_file, edit_tex1_file, edit_tex2_file,
         frame_edit_index, output_folder, video_name, edit_frame_foreground, edit_frame_background, device):
    ...
    for i in range(number_of_frames):
        file1 = input_files[i]
        im = np.array(Image.open(str(file1)))
        video_or_frames.append(im)
...
#       video_frames[:, :, :, i] = torch.from_numpy(cv2.resize(im[:, :, :3], (resx, resy)))
    video_or_frames = np.array(video_or_frames)
    ...
    if frame_edit:
        ...
        print(edit_im1.shape, edit_im1.dtype, np.min(edit_im1), np.max(edit_im1))
...
#               video_frames, video_or_frames,
#               output_folder_final, mask_frames, video_name, evaluate_all_pixels=False,
#               texture_edit_im1=edit_im1,
#               texture_edit_im2=edit_im2, alpha_im1=alpha_im1, alpha_im2=alpha_im2)

    if edit_frame_foreground:
        edit_im = edit_im1
        alpha_im = alpha_im1
        model_F_mapping = model_F_mapping1
    else:
        edit_im = edit_im2
        alpha_im = alpha_im2
        model_F_mapping = model_F_mapping2
    apply_edit_for_layer(model_F_atlas, resx, resy, number_of_frames,
                            video_frames, video_or_frames,
                            model_F_mapping2, model_F_mapping, model_alpha,
                            output_folder_final, mask_frames, video_name,
                            edit_im, alpha_im, edit_frame_foreground)

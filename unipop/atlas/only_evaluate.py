...
from unipop.atlas.unwrap_utils import load_input_data
from unipop.atlas.evaluate import evaluate_model


device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")

def main(training_folder,  output_folder, runinng_command):
    # read config:
    ...
    Path(os.path.join(output_folder, '%s_%06d' % (vid_name, folder_time))).mkdir(parents=True, exist_ok=True)
    file1 = open("%s/runinng_command" % os.path.join(output_folder, '%s_%06d' % (vid_name, folder_time)), "w")
    file1.write(runinng_command)
    file1.close()
    start_iteration = checkpoint["iteration"]
    # run evaluation:
    evaluate_model(model_F_atlas,resx,resy,number_of_frames,model_F_mapping1,model_F_mapping2,model_alpha,video_frames,os.path.join(output_folder, '%s_%06d' % (vid_name, folder_time)),start_iteration,mask_frames,0,0,vid_name,derivative_amount,uv_mapping_scale,optical_flows,optical_flows_mask,device,save_checkpoint=False,show_atlas_alpha=True)
...
if __name__ == "__main__":
    ...
    main(training_folder, output_folder, ' '.join(sys.argv))
